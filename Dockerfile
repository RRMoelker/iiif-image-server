...
RUN cd /tmp/cantaloupe-develop && export MAVEN_OPTS="-Dhttp.proxyHost=10.240.2.1 -Dhttp.proxyPort=8080 -Dhttp.nonProxyHosts=localhost|127.0.0.1" && env && whoami && mvn clean package -DskipTests
RUN cd /usr/local \
